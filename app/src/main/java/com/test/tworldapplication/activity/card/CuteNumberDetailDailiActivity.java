...
public class CuteNumberDetailDailiActivity extends BaseActivity implements PlkBleConnectCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        preRandomNumber = (PreRandomNumber) getIntent().getSerializableExtra("data");
        dialog.getTvTitle().setText("正在获取信息");
        btAdapt = BluetoothAdapter.getDefaultAdapter();// 初始化本机蓝牙功能
        bleService = new PlkBleService(CuteNumberDetailDailiActivity.this);
        getData();
//        initView();
    }

    private void getData() {
        Util.createToast(CuteNumberDetailDailiActivity.this, "正在查询,请稍后");
        PostPreNumberDetails postPreNumberDetails = new PostPreNumberDetails();
        postPreNumberDetails.setSession_token(Util.getLocalAdmin(CuteNumberDetailDailiActivity.this)[0]);
        postPreNumberDetails.setNumber(preRandomNumber.getNum());
        ...
        new CardHttp().preNumberDetails(httpPost, new Subscriber<HttpRequest<RequestPreNumberDetails>>() {
            @Override
            ...
            public void onNext(HttpRequest<RequestPreNumberDetails> requestPreNumberDetailsHttpRequest) {
                Util.createToast(CuteNumberDetailDailiActivity.this, requestPreNumberDetailsHttpRequest.getMes());
        ...
        switch (view.getId()) {
            case R.id.tvNext:
                if (!Util.isFastDoubleClick()) {
                    if (tvIccid.getVisibility() == View.GONE) {
                        if (mPackage == null || mPromotion == null) {
                            Toast.makeText(CuteNumberDetailDailiActivity.this, "请选择套餐和活动包!", Toast.LENGTH_SHORT).show();
                        } else {
                    /*提交白卡预开户*/
                            PostWritePreDetails postWritePreDetails = new PostWritePreDetails();
                            postWritePreDetails.setSession_token(Util.getLocalAdmin(CuteNumberDetailDailiActivity.this)[0]);
                            ...
                            new CardHttp().writePreDetails(httpPost, new Subscriber<HttpRequest>() {
                                ...
                                public void onNext(HttpRequest httpRequest) {
                                    Toast.makeText(CuteNumberDetailDailiActivity.this, httpRequest.getMes(), Toast.LENGTH_SHORT).show();
                                    if (httpRequest.getCode() == BaseCom.NORMAL) {
                                        Intent intent = new Intent(CuteNumberDetailDailiActivity.this, CuteNumberWriteSuccessActivity.class);
...
//                Intent intent = new Intent(this, CuteNumberDetailDailiActivity.class);
//                intent.putExtra("data", requestPreNumberDetails);
//                startActivity(intent);
//                overridePendingTransition(R.anim.zoomin, R.anim.zoomout);
                }
            ...
            case R.id.ll_package:
                if (!Util.isFastDoubleClick()) {
                    mRequestImsi = new RequestImsi();
                    mRequestImsi.setPackages(requestPreNumberDetails.getPackages());
                    Intent intent0 = new Intent(CuteNumberDetailDailiActivity.this, PackageSelectDetailActivity.class);
                    Bundle bundle = new Bundle();
                    bundle.putSerializable("imsi", mRequestImsi);
            ...
            case R.id.ll_activity:
                if (!Util.isFastDoubleClick()) {
                    if (mPackage == null) {
                        Util.createToast(CuteNumberDetailDailiActivity.this, "请选择套餐!");
                    } else {
                        Intent intent1 = new Intent(CuteNumberDetailDailiActivity.this, ActivitySelectDetailActivity.class);
                        ...
                        public void onAnimationEnd(Animation animation) {
                            ...
                            if (mPackage == null || mPromotion == null) {
                                ...
                                if (btAdapt.isEnabled()) {
                                    if (mac[1].equals("")) {
                /*蓝牙地址未存本地,跳转选择蓝牙设备界面*/
                                        Intent serverIntent2 = new Intent(CuteNumberDetailDailiActivity.this, BlueToothReadActivity.class);
                                        ...
                                        try {
                                            if (receive == 0) {
                                                ...
                                                dialog.show();
                                                bleService.connectDevice(mac[1], CuteNumberDetailDailiActivity.this);
                                            } else {
...
/*读iccid*/
                                    ...
                                    /*根据iccid获取imsi*/
                                                            ...
                                                            new CardHttp().lockNumber(CardRequest.lockNumber(CuteNumberDetailDailiActivity.this, new SuccessNull() {
                                                                @Override
...
//
                                                            ...
                                                            e.printStackTrace();
                                                            Toast.makeText(CuteNumberDetailDailiActivity.this, e.getMessage(), Toast.LENGTH_LONG)
                                                                    .show();
                                                        }
...
//                                addLog(String.valueOf(chars), 2);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (data != null) {
            switch (requestCode) {
                case 0:
                    mPackage = mRequestImsi.getPackages()[data.getIntExtra("select", 0)];
...
//                    tvNext.setVisibility(View.VISIBLE);
                ...
                case 12:
                    ...
                    try {
                        bleService.connectDevice(address, CuteNumberDetailDailiActivity.this);
                    } catch (PlkException e) {
                        e.printStackTrace();
                    }
                    break;
            ...
            if (requestCode == REQUEST_ENABLE_BT) {
                if (resultCode == RESULT_OK) {
                    ...
                    receive = 0;
                    Toast.makeText(CuteNumberDetailDailiActivity.this, "请开启蓝牙", Toast.LENGTH_SHORT).show();
//                    finish();
                }
            }
//            dialog.dismiss();
    ...
    public void getImsi() {
        ...
        postImsi.setSession_token(Util.getLocalAdmin(CuteNumberDetailDailiActivity.this)[0]);
        ...
        new CardHttp().getPreImsi(httpPost, new Subscriber<HttpRequest<RequestImsi>>() {
            ...
            public void onNext(HttpRequest<RequestImsi> requestImsiHttpRequest) {
                Toast.makeText(CuteNumberDetailDailiActivity.this, requestImsiHttpRequest.getMes(), Toast.LENGTH_SHORT).show();
...
//                                llShow.setVisibility(View.VISIBLE);
                                ...
                                Toast.makeText(CuteNumberDetailDailiActivity.this, "写卡失败!", Toast.LENGTH_SHORT)
                                        .show();
//                                                                    tvSubmit.setVisibility(View.VISIBLE);
    ...
    public void onConnectSuccess() {
        ...
        Log.d("aaa", "success");
        receive = 1;
        Util.createToast(CuteNumberDetailDailiActivity.this, "连接设备成功,请再次点击写卡按钮");
    }
    ...
    public void onConnectFailed(String s) {
        receive = 0;
        dialog.dismiss();
        Util.createToast(CuteNumberDetailDailiActivity.this, "设备连接失败,请重新连接!");
        clearAddressmac();
    }
