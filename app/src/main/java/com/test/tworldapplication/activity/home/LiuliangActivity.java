...
import com.test.tworldapplication.entity.ProductBean;
...
public class LiuliangActivity extends BaseActivity {
    ...
    TextView tvTitle;
    List<ProdBean> prodBeans = new ArrayList<>();
...
//        phone = "17091500119";
        ...
        adapter = new RecyclerAdapter(new Callback() {
            ...
            public void callback(int groupPosition, int childPosition) {
                ...
                serviceProduct.setSession_token(Util.getLocalAdmin(LiuliangActivity.this)[0]);
                ...
                new AccountHttp()._2019ServiceProductDetails(new Subscriber<HttpRequest<RequestServiceProductDetail>>() {
                    ...
                    public void onNext(HttpRequest<RequestServiceProductDetail> httpRequest) {
                        if (httpRequest.getCode() == BaseCom.NORMAL) {
                            isCancelFlag = httpRequest.getData().getProducts().getIsCancleFlag();
                            initCheckBuyDialog();
                        } else
                            Toast.makeText(LiuliangActivity.this, httpRequest.getMes(), Toast.LENGTH_SHORT).show();

                    }
                }, httpPost);


//                for (int i = 0; i < prodBeans.size(); i++) {
//                    ProdBean chooseProdBean = prodBeans.get(i);
//
////                    if (i == groupPosition) {
////                        prodBean.setIsSelect(1);
////                        List<ProductBeanDetail> productBeanDetails = prodBean.getProds();
////                        for (int j = 0; j < productBeanDetails.size(); j++) {
////                            ProductBeanDetail productBeanDetail = productBeanDetails.get(j);
////                            if (j == childPosition) {
////                                productBeanDetail.setIsSelect(1);
////                            } else
////                                productBeanDetail.setIsSelect(0);
////                            productBeanDetails.set(j, productBeanDetail);
////                        }
////                        prodBean.setProds(productBeanDetails);
////
////                    } else {
////                        prodBean.setIsSelect(0);
////                        List<ProductBeanDetail> productBeanDetails = prodBean.getProds();
////                        for (int j = 0; j < productBeanDetails.size(); j++) {
////                            ProductBeanDetail productBeanDetail = productBeanDetails.get(j);
////                            productBeanDetail.setIsSelect(0);
...
//        recyclerView.setLayoutManager(new LinearLayoutManager(LiuliangActivity.this));
    ...
    public void initCheckBuyDialog() {
        checkBuyDialog = new Dialog(LiuliangActivity.this);
        checkBuyDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        checkBuyDialog.setContentView(R.layout.dialog_buy_or_retreat);
        ...
        ImageView imgClose = (ImageView) checkBuyDialog.findViewById(R.id.imgClose);
        ...
        TextView tvDivide = (TextView) checkBuyDialog.findViewById(R.id.tvDivide);
        tvTitle.setText(chooseProductBeanDetail.getName());
    ...
    public void initRetreatToastDialog() {
        retreatToastDialog = new Dialog(LiuliangActivity.this);
        retreatToastDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        retreatToastDialog.setContentView(R.layout.dialog_retreat_toast);
        retreatToastDialog.setCanceledOnTouchOutside(true);
        Window window = retreatToastDialog.getWindow();
        WindowManager.LayoutParams layoutParams = window.getAttributes();
        layoutParams.gravity = Gravity.CENTER;
        layoutParams.width = WindowManager.LayoutParams.MATCH_PARENT;
        layoutParams.height = WindowManager.LayoutParams.MATCH_PARENT;
        ...
        window.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT)); //背景透明
        window.setAttributes(layoutParams);
        TextView tvTitle = (TextView) retreatToastDialog.findViewById(R.id.tvTitle);
        TextView tvContent = (TextView) retreatToastDialog.findViewById(R.id.tvContent);
        TextView tvName = (TextView) retreatToastDialog.findViewById(R.id.tvName);
        TextView refuse = (TextView) retreatToastDialog.findViewById(R.id.refuse);
        TextView agree = (TextView) retreatToastDialog.findViewById(R.id.agree);
        tvTitle.setText("提示");
        tvContent.setText("确认退订该产品");
        ...
        agree.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                serviceProduct.setSession_token(Util.getLocalAdmin(LiuliangActivity.this)[0]);
                serviceProduct.setType(2);
                serviceProduct.setProductId(chooseProductBeanDetail.getProductId());
                serviceProduct.setNumber(phone);
                serviceProduct.setOperateType(0);
        ...
        retreatToastDialog.show();
    }

    public void initBuyInfoDialog() {
        buyInfoDialog = new Dialog(LiuliangActivity.this);
        buyInfoDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        buyInfoDialog.setContentView(R.layout.dialog_buy_info);
        ...
        tvName.setText("价格：" + chooseProductBeanDetail.getOrderAmount() + "元");
        tvContent.setText(chooseProductBeanDetail.getName());
        refuse.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
    ...
    private void getData() {
        ...
        postQueryProduct.setSession_token(Util.getLocalAdmin(LiuliangActivity.this)[0]);
        postQueryProduct.setType(2);
        httpPost.setApp_key(Util.encode(BaseCom.APP_KEY));
        ...
        new AccountHttp()._2019QueryProducts(new Subscriber<HttpRequest<ProductListEntity>>() {
            ...
            public void onNext(HttpRequest<ProductListEntity> productListEntityHttpRequest) {
                Util.createToast(LiuliangActivity.this, productListEntityHttpRequest.getMes());
                prodBeans.clear();
                if (productListEntityHttpRequest.getCode() == BaseCom.NORMAL) {
                    if (productListEntityHttpRequest.getData().getProducts().getDataProds().size() > 0) {
                        prodBeans.addAll(productListEntityHttpRequest.getData().getProducts().getDataProds());
                    ...
                    Util.gotoActy(LiuliangActivity.this, LoginActivity.class);
            }
        }, httpPost);
    }
    ...
    class RecyclerAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = LayoutInflater.from(LiuliangActivity.this).inflate(R.layout.item_liuliang, null);
    ...
    class GvAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = LayoutInflater.from(LiuliangActivity.this).inflate(R.layout.adapter_account_recharge, null);
